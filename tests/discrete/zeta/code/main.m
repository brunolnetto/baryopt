close all
clear all
clc

n = 2;

init_val = 0;

% Method hyperparameter
nu = 5;
sigma = 1;
lambda = 1;

% Recursive version
syms x y

x_0 = 1;
y_0 = -1;

func = (x - x_0)^2 + (y - y_0)^2;
oracle = @(x) (x(1) - x_0)^2 + (x(2) - y_0)^2;

x0 = init_val*zeros(n, 1);
m0 = 0;

iterations = 25;
n_iterations = 150;

zetas = {1, 0.66 0.33};

% Accelerated values
zbars_zetas = {};
xs_zetas = {};
deltas_zetas = {};

msg = sprintf('Calculating zetas');
wb_zetas = my_waitbar(msg);

for j = 1:length(zetas)
    zeta = zetas{j};
    
    zbars_acc = zeros(iterations, 2);
    x_acc = zeros(iterations, 2);
    deltas_acc = zeros(iterations-1, 2);
    
    msg = sprintf('zeta := %.2f, i = %d', zeta, i);
    wb_zeta = my_waitbar(msg);
    for i = 1:n_iterations
        msg = sprintf('zeta := %.2f, i = %d', zeta, i);
        wb_zeta.change_title(msg);
        
        accel_fun = @(m_1, xhat_1, delta_xhat_1) ...
            pait_accel(m_1, xhat_1, delta_xhat_1, zeta);
        
        [~, xhats, ~, ~, deltas, zbars, ~, ~] = ...
            drecexpbary_custom(oracle, m0, x0, ...
                               nu, sigma, ...
                               lambda, iterations, ...
                               accel_fun, ...
                               struct('verbose', true));
        
        clear(func2str(@integrated_accel));
        
        xhats = xhats(1:end-1, :);
        zbars = zbars(1:end-1, :);
        deltas = deltas(1:end-1, :);
        
        zbars_acc = zbars_acc + zbars;
        x_acc = x_acc + xhats;
        deltas_acc = deltas_acc + deltas;

        wb_zeta = wb_zeta.update_waitbar(i, n_iterations);
    end
    
    wb_zeta.close_window();
    
    xs_zetas{end+1} = x_acc/n_iterations;
    zbars_zetas{end+1} = zbars_acc/iterations;
    deltas_zetas{end+1} = deltas_acc/iterations;
    
    wb_zetas = wb_zetas.update_waitbar(j, length(zetas));
end

wb_zetas.close_window();

zetas = cellfun(@num2str, zetas, 'UniformOutput', false);
markers_ = {'k-', 'r--', '.b-', 'sg'};
markers = {markers_{1:length(zetas)}};

% Delta x plot
plot_config.titles = {'', ''};
plot_config.xlabels = {'', 'Iterations'};
plot_config.ylabels = {'$\Delta \hat{x}_1(\zeta)$', ...
                       '$\Delta \hat{x}_2(\zeta)$'};
plot_config.grid_size = [2, 1];
plot_config.plot_type = 'stem';
plot_config.legends = {zetas, zetas};
plot_config.pos_multiplots = [repmat(1, 1, length(zetas)-1), ...
                              repmat(2, 1, length(zetas)-1)];
plot_config.markers = {markers, markers};
plot_config.axis_style = 'square';

delta_first = [deltas_zetas{1}(:, 1), ...
               deltas_zetas{1}(:, 2)];
delta_multi = [deltas_zetas{2}(:, 1), ...
               deltas_zetas{3}(:, 1), ...
               deltas_zetas{2}(:, 2), ...
               deltas_zetas{3}(:, 2)];

deltas = {delta_first, delta_multi};

iters = 1:length(delta_first(:, 1));
[hfigs_delta, axs] = my_plot(iters, deltas, plot_config);

axs{1}{1}.Legend.Location = 'bestoutside';
axs{1}{2}.Legend.Location = 'bestoutside';

% zbar plot
plot_config.titles = {'', ''};
plot_config.xlabels = {'', 'Iterations'};
plot_config.ylabels = {'$\bar{z}_1$', '$\bar{z}_2$'};
plot_config.grid_size = [2, 1];
plot_config.plot_type = 'stem';
plot_config.legends = {zetas, zetas};
plot_config.pos_multiplots = [repmat(1, 1, length(zetas)-1), ...
                              repmat(2, 1, length(zetas)-1)];
plot_config.markers = {markers, markers};
plot_config.axis_style = 'square';

n = length(zbars_zetas{1}(:, 1));
zbars_first = [zbars_zetas{1}(:, 1), ...
               zbars_zetas{1}(:, 2)];
zbars_multi = [zbars_zetas{2}(:, 1), ...
               zbars_zetas{3}(:, 1), ...
               zbars_zetas{2}(:, 2), ...
               zbars_zetas{3}(:, 2)];

zs = {zbars_first, zbars_multi};
           
iters = 1:length(zbars(:, 1));
[hfigs_z, axs] = my_plot(iters, zs, plot_config);

axs{1}{1}.Legend.Location = 'bestoutside';
axs{1}{2}.Legend.Location = 'bestoutside';

% x-coordinate plot
plot_config.titles = {'', ''};
plot_config.xlabels = {'', 'Iterations'};
plot_config.ylabels = {'$\hat{x}_1(\lambda_z)$', ...
                       '$\hat{x}_2(\lambda_z)$'};
plot_config.grid_size = [2, 1];
plot_config.plot_type = 'stem';
plot_config.legends = {zetas, zetas};
plot_config.pos_multiplots = [repmat(1, 1, length(zetas)-1), ...
                              repmat(2, 1, length(zetas)-1)];
plot_config.markers = {markers, markers};
plot_config.axis_style = 'square';

x_first = [xs_zetas{1}(:, 1), ...
           xs_zetas{1}(:, 2)];
x_multi = [xs_zetas{2}(:, 1), xs_zetas{3}(:, 1), ...
           xs_zetas{2}(:, 2), xs_zetas{3}(:, 2)];

xs = {x_first, x_multi};

iters = 1:length(xs_zetas{1}(:, 1));
[hfigs_x_x0, axs] = my_plot(iters, xs, plot_config);

axs{1}{1}.XLim = [0 iterations];
axs{1}{2}.XLim = [0 iterations];
axs{1}{1}.Legend.Location = 'bestoutside';
axs{1}{2}.Legend.Location = 'bestoutside';

% Save folder
path = [pwd '/../imgs/'];
fname = 'delta';
saveas(hfigs_delta, [path, fname], 'epsc');

fname = 'zbar_';
saveas(hfigs_z, [path, fname], 'epsc');

fname = 'xbars_';
saveas(hfigs_x_x0, [path, fname], 'epsc');


